...
# Final boss — Duryudana. Struktur file ini SENGAJA dibuat mirip banget
# sama dursasana.py (yang juga mirip miniboss1.py), sesuai pola boss
# fight yang sudah ada. Bedanya: HP jauh lebih banyak (ini final boss,
# di atas Dursasana), nama, dan cara dipanggil dari main.py (fixed di
# Round 5, sekali saja, tidak random & tidak berulang).
...
def _load_font(size):
    ...
    try:
        ...
        print(f"[DURYUDANA] Font '{FONT_PATH}' tidak ditemukan — pakai font default sementara")
        return pygame.font.SysFont(None, size)


def _draw_loading_bar(screen, progress, label=""):
    """
    Gambar layar loading sederhana (progress bar) sambil asset Duryudana
    ...
    pygame.draw.rect(screen, (60, 15, 60), (bar_x - 3, bar_y - 3, bar_w + 6, bar_h + 6), border_radius=8)
    pygame.draw.rect(screen, (30, 10, 30), (bar_x, bar_y, bar_w, bar_h), border_radius=6)

    fill_w = int(bar_w * max(0.0, min(1.0, progress)))
    if fill_w > 0:
        pygame.draw.rect(screen, (180, 30, 180), (bar_x, bar_y, fill_w, bar_h), border_radius=6)

    pygame.draw.rect(screen, (150, 70, 150), (bar_x, bar_y, bar_w, bar_h), 2, border_radius=6)

    title_surf = _load_font(24).render("Memuat Final Boss Duryudana...", True, (230, 210, 255))
    screen.blit(title_surf, title_surf.get_rect(center=(w // 2, bar_y - 28)))

    if label:
        label_surf = _load_font(18).render(label, True, (190, 170, 210))
...
def load_spritesheet_row(path, frame_count, scale=1):
    """Sama seperti versi di dursasana.py: kalau file spritesheet health
    bar belum ada, generate frame placeholder warna solid supaya game
    tidak crash sebelum asset duryudana disiapkan."""
    if not os.path.exists(path):
        print(f"[DURYUDANA] Healthbar '{path}' tidak ditemukan — pakai placeholder sementara")
        frames = []
        for i in range(frame_count):
            ratio = 1 - (i / max(1, frame_count - 1))
            surf = pygame.Surface((220, 24), pygame.SRCALPHA)
            color = (int(80 + 100 * ratio), int(15 + 10 * ratio), int(80 + 100 * ratio), 255)
            surf.fill(color)
            frames.append(surf)
        return frames
...
class Duryudana(pygame.sprite.Sprite):
    def __init__(self, x, y, assets_folder='assets2/final_boss', screen=None):
        super().__init__()
        ...
        walk_frames = load_frames(f"{assets_folder}/final-boss-walk", 0.3)
        ...
        if not startfight_frames:
            ...
            print("[DURYUDANA] StartFight folder tidak ditemukan — pakai Walk frames sebagai fallback")

        _draw_loading_bar(screen, 2 / _TOTAL_STEPS, "Animasi mati...")
        die_frames = load_frames(f"{assets_folder}/final-boss-death", 0.6)
        # Kalau asset "final-boss-death" belum ada, dipakai animasi mati
        # prosedural (fade + menciut + tenggelam), sama seperti Dursasana.
        self.has_real_die_anim = bool(die_frames)
        if not self.has_real_die_anim:
            print("[DURYUDANA] Die folder tidak ditemukan — pakai animasi mati prosedural (fade/shrink)")

        _draw_loading_bar(screen, 3 / _TOTAL_STEPS, "Animasi serang...")
        attack_frames = load_frames(f"{assets_folder}/final-boss-attack", 0.7)

        # ── Animasi dash — kalau folder belum ada, pakai walk frames
        #    sebagai fallback biar tetap ada gambar saat nge-dash ──────
        dash_frames = load_frames(f"{assets_folder}/final-boss-dash", 0.3)
        if not dash_frames:
            dash_frames = walk_frames
            print("[DURYUDANA] Dash folder tidak ditemukan — pakai Walk frames sebagai fallback")
        ...
        # ── Health bar sprite — final boss punya health bar sendiri,
        #    segmennya lebih banyak lagi dibanding Dursasana karena HP
        #    jauh lebih besar ────────────────────────────────────────
        self.healthbar_frames = load_spritesheet_row(
            "assets2/healthbar_duryudana.png", 10, scale=2
        )
        ...
        # ── Stats — HP paling banyak sejauh ini, ini final boss ──────────
        self.max_health = 160
        ...
        # ── Intro state — boss diam & tidak bisa diserang selama intro ──
        ...
        self.dash_cooldown     = 220       # jeda sebelum bisa dash lagi
        self.dash_timer        = 0
    ...
    def get_attack_hitbox(self):
        ...
        # jalan sedikit, hitbox aktif SEKALI per dash — dipakai rect
        # Duryudana sendiri yang sedikit di-inflate.
        if self.dashing:
    ...
    def _animate_die_frames(self):
        """Animasi mati berbasis sprite asli (folder boss-death ada)."""
    ...
    def _animate_procedural_death(self):
        ...
        "boss-death". Sama seperti Dursasana: fade out + shrink +
        tenggelam + tint ungu (beda dari tint merah Dursasana), dari
        frame terakhir sebelum mati.
        """
    ...
    # atas layar tetap digambar oleh main.py, sama seperti Dursasana.
    # =================================================================
